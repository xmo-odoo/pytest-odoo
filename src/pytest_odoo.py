    ...
    def load_module_code(cr: Cursor) -> None:
        ...
        while previously_processed < len(processed_modules):
            ...
            processed_modules += loading.load_marked_modules(
                env, g, ['installed'], (), None, None, loaded_modules, False
            )
...
def pytest_addoption(parser: pytest.Parser) -> None:
    ...
    parser.addoption(
        '--non-standard',
        action="store_true",
        help="runs tests which are not marked 'standard', they are skipped by default",
    )
    parser.addoption(
        '--non-post',
        action="store_true",
        help="runs tests which are not marked as 'post_install', they are skipped by default",
    )
...
def pytest_configure(config: pytest.Config) -> None:
    ...
    if config.getini("consider_namespace_packages"):
        raise ValueError("namespace packages should not be enabled to test odoo")
...
def _get_default_datadir() -> str:
    ...
    if os.path.isdir(home):
        return appdirs.user_data_dir(appname="Odoo", appauthor="Odoo S.A.")
    elif sys.platform in ['win32', 'darwin']:
        return appdirs.site_data_dir(appname="Odoo", appauthor="Odoo S.A.")
    else:
        return "/var/lib/odoo"
...
def odoo_http(odoo_session: registry.Registry) -> Iterator[None]:
    ...
    with odoo_session.cursor() as cr:
        odoo.api.Environment(cr, odoo.SUPERUSER_ID, {})['ir.qweb']._pregenerate_assets_bundles()
...
def import_path(
    path: str | os.PathLike[str], *, mode: str, root: Path, consider_namespace_packages: bool
) -> ModuleType:
    path = Path(path)
    if not path.exists():
...
def pytest_collection_modifyitems(config: pytest.Config, items: list[pytest.Item]) -> None:
    ...
    for item in items:
        ...
        # marking needs to be done on the item to avoid inheritance
        # issues mucking things up, as markers are additive through
        # inheritance
        #
        # also we want to skip free functions (e.g. standalone) and
        # non-odoo test cases
        ...
        # JS tests functions require a `_test_params`, add a fake one
        # here for now (maybe JS tests could be treated as parametric
        # eventually?)
